...
// the method outputs the date as a fixed string
// the method is used to output date along with other informnation in columns 
// so that all data is lined up
string Date::dateString() const
{
   stringstream out;
   out << *this; // resulting string is what the outputFormat is set to
   string str = out.str();
   return str;
}
